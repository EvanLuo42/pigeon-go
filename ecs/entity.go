...
// store entities in slices, and use the P=n*log n lookup for them
...
// EG:
// s.AddByInterface(an interface{GetBasicEntity()*BasicEntity, GetSpaceComponent()*SpaceComponent){
...
func (e *BasicEntity) RemoveChild(child *BasicEntity) {
	...
	if d >= 0 {
		e.children = append(e.children[:d], e.children[d+1:]...)
	}
}
...
// BasicFace BasicFase is an interface that BasicEntity and entities containing
